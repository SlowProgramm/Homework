...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for el in range(0, len(category)):
            cater = Category.objects.create(title=category[el], color=colors[el])
            author = Author.objects.create(username=authors[el])
            song = Songs.objects.create(name=name[el], category=cater, author=author)
        ...
        for song in songs:
            ...
            print('|', song_name.center(10), '|', song_category.center(10), '|', song_author.center(10), '|')
